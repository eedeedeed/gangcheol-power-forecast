...
import ProfilePage from './components/ProfilePage'; // ProfilePage 임포트
import AuthPage from './components/AuthPage';     // AuthPage 임포트

function App() {
  // 로그인 상태를 관리하는 state입니다. 로컬 스토리지에 'token'이 있으면 로그인된 상태로 시작합니다.
  const [isLoggedIn, setIsLoggedIn] = useState(!!localStorage.getItem('token'));
  // 현재 활성화된 탭을 관리하는 state입니다. 로그인 상태에 따라 초기 탭을 설정합니다.
  const [activeTab, setActiveTab] = useState(isLoggedIn ? 'dashboard' : 'auth');

  // 데이터 시뮬레이션 및 상태 관리 (기존 로직 유지)
  ...
  const [savingsGuides, setSavingsGuides] = useState(powerData.savings_guide); // savings_guide 상태 추가
  const [selectedBuildingId, setSelectedBuildingId] = useState(1); // 선택된 건물 ID 상태 추가

  // 실시간 전력 사용량 시뮬레이션 및 데이터 업데이트를 위한 useEffect 훅입니다.
  useEffect(() => {
  ...
  return (
    // 애플리케이션의 최상위 컨테이너입니다. (기존 디자인 유지를 위해 'app' 클래스 사용)
    <div className="app">
      {/* 상단 내비게이션 바 컴포넌트입니다. 로그인 상태와 관계없이 항상 렌더링됩니다. */}
        ...
        <main className="main-content">
          ...
          {isLoggedIn ? (
            <>
              {activeTab === 'dashboard' && (
                <Dashboard
                  currentPower={{ usage: currentUsage, percentage: usagePercentage, capacity: powerData.current_power.capacity }}
            ...
            // AuthPage가 main-content 내부에 렌더링되도록 하여 기존 레이아웃을 유지합니다.
